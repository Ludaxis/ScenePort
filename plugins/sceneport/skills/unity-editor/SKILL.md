...
5. After code or scene changes, read `unity_audit_log`, `unity_get_compilation_status`, console logs, and run relevant Unity tests when available.
...
When ScenePort is unavailable, explain that the Unity bridge package must be installed in the Unity project, the MCP server must be built with `npm run build`, and `sceneport doctor` should be run from the Unity project root or with `SCENEPORT_PROJECT_PATH` set.
